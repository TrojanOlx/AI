...
{
    public class Service_user_face_compare_prepayment
    {
        static string Select = @"SELECT
                                    public.user_face_compare_prepayment.id,
                                    public.user_face_compare_prepayment.user_id,
                                    public.user_face_compare_prepayment.number,
                                    public.user_face_compare_prepayment.created_at,
                                    public.user_face_compare_prepayment.updated_at
                                FROM
                                    public.user_face_compare_prepayment
                                WHERE
                                    1 = 1 ";

        static string Update = @"UPDATE public.user_face_compare_prepayment
                                 SET 
                                    {0} 
                                 WHERE 
                                    1=1 ";
